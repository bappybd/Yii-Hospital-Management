...
class CustomerInvoiceController extends Controller
{
   ...
   public function accessRules()
   {
      return array(
         ...
         array('allow', // allow authenticated user to perform 'create' and 'update' actions
            'actions'=>array('create','update','DiagonisticEntryForm', 'InvoiceMemo','InvoiceTestsList'),
            'users'=>array('@'),
         ),
         array('allow', // allow admin user to perform 'admin' and 'delete' actions
            'actions'=>array('admin','delete','UpdateCustomerInvoiceTests','InvoiceTestsList'),
            'users'=>array('admin'),
         ),
    ...
    * Manages all models.
    */
   public function actionInvoiceTestsList()
   {
      $model = new PatientTracker('search');
      $model->unsetAttributes();  // clear any default values
      if(isset($_GET['CustomerInvoice']))
         $model->attributes=$_GET['CustomerInvoice'];

      $this->render('invoiceTestsList',array(
         'model'=>$model,
      ));
   }
   
   public function actionUpdateCustomerInvoiceTests()
   {
      $models = CustomerInvoice::model()->with('patientTracker')->findAll();
      
      foreach($models as $model){
         if(isset($model->patientTracker)){
            echo "<br />$model->patient_id : ".$totalTests = count($model->patientTracker); 
            $model->total_tests = $totalTests;
            $model->save();
         }
      }
   }
